...
#' @export
margin_plot_varying_expected_error_rate <- function(allowed_error_rate,
                                                    cert = 0.95,
                                                    max_n = 1000,
                                                    S = 10000) {
  # Argument check.
  ...
  # Combine expected_error_rate, draws_needed and possible into one tibble.
  ...
  result <-
    ...
    geom_point(mapping = aes(x = expected_error_rate, y = draws_needed, color = possible)) +
